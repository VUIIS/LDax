    ...
    def __init__(self, processor, assessor, upload_dir):
        ...
        if not assessor.exists():
            ...
            if atype == DEFAULT_DATATYPE.lower():
                #LOGGER.warn('SHUNXING self.get_processor_version:%s' %str(self.get_processor_version()))
                assessor.attrs.mset(
        ...
        #LOGGER.warn('SHUNXING budongle self.assessor_id: %s' % str(assessor.id()))
        #LOGGER.warn('SHUNXING budongle self.assessor_labale: %s' % str(assessor.label()))
...
##### COMMENT BY SHUNXING        return date_str
    ...
    def set_createdate_today(self):
        ...
        """
        today_str = str(date.today())
        #LOGGER.warn('SHUNXING today_str:%s' % str(today_str))
    ...
    def get_statuses(self):
        ...
        :return: Serially ordered strings of the assessor procstatus,
         qcstatus, then jobid.
        """
        #LOGGER.warn('SHUNXING task.get_statuses 1')
        if not self.assessor.exists():
            #LOGGER.warn('SHUNXING task.get_statuses 2')
            xnat_status = DOES_NOT_EXIST
            qcstatus = DOES_NOT_EXIST
            jobid = ''
        elif self.atype.lower() in [DEFAULT_DATATYPE.lower(),
                                    DEFAULT_FS_DATATYPE.lower()]:
            #LOGGER.warn('SHUNXING task.get_statuses 3')
            ...
            # comment 
            #f = open('self.assessor.attrs_'+str(self.atype)+'.txt', 'w' )
            #f.write(str(self.assessor.attrs.mget) + '\n')
            #f.close()

        else:
            #LOGGER.warn('SHUNXING task.get_statuses 4')
            xnat_status = 'UNKNOWN_xsiType: %s' % self.atype
            qcstatus = 'UNKNOWN_xsiType: %s' % self.atype
            jobid = ''

        
        #LOGGER.warn('SHUNXING xnat_status:%s' % str(xnat_status))
        #LOGGER.warn('SHUNXING qcstatus:%s' % str(qcstatus))
        #LOGGER.warn('SHUNXING jobid:%s' % str(jobid))
        #LOGGER.warn('SHUNXING self.atype:%s' % str(self.atype))
        #LOGGER.warn('SHUNXING self.assessor.attrs.mget:%s' % str(self.assessor.attrs) )
    ...
    def set_proc_and_qc_status(self, procstatus, qcstatus):
        ...
        """
        #LOGGER.warn('SHUNXING set_proc_and_qc_status')
        self.assessor.attrs.mset({
...
class ClusterTask(Task):
    ...
    def set_proc_and_qc_status(self, procstatus, qcstatus):
        ...
        Set the procstatus and qcstatus of the assessor
        """
        #LOGGER.warn('SHUNXING set_proc_and_qc_status not implement')
        raise NotImplementedError()
...
undo_processing...')
    ...
    def build_task(self, csess, old_proc_status,old_qc_status, 
        ...
        ### EDIT by SHUNXING
        """
        #LOGGER.warn('SHUNXING self.assessor: %s' % str(self.assessor) )
        #print(dir(self.assessor))
        #LOGGER.warn('SHUNXING self.assessor_label: %s' % str( self.assessor.info))
        LOGGER.warn('SHUNXING XnatTask.build_task 1')
        #LOGGER.warn('SHUNXING before get_statuses#################')
        #LOGGER.warn('SHUNXING old_proc_status:%s' % str(old_proc_status))
        #LOGGER.warn('SHUNXING old_qc_status:%s' % str(old_qc_status))

        #(old_proc_status, old_qc_status, _) = self.get_statuses()

        #LOGGER.warn('SHUNXING after get_statuses#################')
        #LOGGER.warn('SHUNXING old_proc_status:%s' % str(old_proc_status))
        #LOGGER.warn('SHUNXING old_qc_status:%s' % str(old_qc_status))
                                
        #LOGGER.warn('SHUNXING old_proc_status:%s' % str(old_proc_status))
        #LOGGER.warn('SHUNXING old_qc_status:%s' % str(old_qc_status))
        #LOGGER.warn('SHUNXING csess.info %s' % str(csess.info()))
        try:
            ...
            LOGGER.warn('SHUNXING XnatTask.build_task 4')
            outlog = self.outlog_path()
            LOGGER.warn('SHUNXING XnatTask.build_task 5')
            ...
            batch.write()
            LOGGER.warn('SHUNXING XnatTask.build_task 6')
            new_proc_status = JOB_RUNNING
            new_qc_status = JOB_PENDING
        except NeedInputsException as e:
            #LOGGER.warn('SHUNXING XnatTask.build_task 7')
            new_proc_status = NEED_INPUTS
            #LOGGER.warn('SHUNXING XnatTask.build_task 8')
            new_qc_status = e.value
        except NoDataException as e:
            #LOGGER.warn('SHUNXING XnatTask.build_task 9')
            new_proc_status = NO_DATA
            new_qc_status = e.value

        #LOGGER.warn('SHUNXING XnatTask.build_task 10')
        if new_proc_status != old_proc_status or \
           new_qc_status != old_qc_status:
            #LOGGER.warn('SHUNXING XnatTask.build_task 11')
            self.set_proc_and_qc_status(new_proc_status, new_qc_status)
            #LOGGER.warn('SHUNXING XnatTask.build_task 12')
    ...
    def build_commands(self, cobj, jobdir):
        ...
        """
        LOGGER.warn('SHUNXING build_commands 1')
        #LOGGER.warn('SHUNXING cobj:%s' % str(cobj))
        #LOGGER.warn('SHUNXING jobdir:%s' % str(jobdir))
        #### LOGGER.warn('SHUNXING self.assessor_label:%s' % str(self.assessor_label))
        #### assr_dir = os.path.join(jobdir, self.assessor_label)
        #LOGGER.warn('SHUNXING self.assessor_label:%s' % str(self.assessor.label()))

        assr_dir = os.path.join(jobdir, self.assessor_label)
        try:
            LOGGER.warn('SHUNXING build_commands 2')
 ...
 #           LOGGER.warn('SHUNXING cobj:%s' % str(cobj))
            ...
            #LOGGER.warn('SHUNXING qcstatus: %s' % str(qcstatus))
            if has_inputs == 1:
                LOGGER.warn('SHUNXING build_command 4')
                LOGGER.warn('nima?????')
                print(os.path.abspath(inspect.getfile(self.processor.get_cmds)))
                return self.processor.get_cmds(self.assessor, assr_dir)
            elif has_inputs == -1:
                LOGGER.warn('SHUNXING build command 5')
                raise NoDataException(qcstatus)
            else:
                LOGGER.warn('SHUNXING build command 6')
